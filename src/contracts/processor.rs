...
// ─── Router, um mini facade dentro do processor
...
fn convert_integer(
    ...
    // switch para qual base usar
    match (source_base, target_base) {
        (2, 8) => Ok(bin_to_oct(digits)),
        (8, 2) => Ok(oct_to_bin(digits)),
        (2, 16) => Ok(bin_to_hex(digits)),
        (16, 2) => Ok(hex_to_bin(digits)),
        // oct <-> hex passam por bin como ponte, sem decimal
        (8, 16) => {
            let bin = oct_to_bin(digits);
            Ok(bin_to_hex(&bin.chars().collect::<Vec<char>>()))
        }
        (16, 8) => {
            let bin = hex_to_bin(digits);
            Ok(bin_to_oct(&bin.chars().collect::<Vec<char>>()))
        }
        _ => Err(ProcessingError::UnsupportedBase(target_base)),
    }
}
...
impl NumericProcessingService for ConversionProcessor {
    fn convert(
        ...
        let output_value = convert_integer(digits, source_base, target_base)?;
